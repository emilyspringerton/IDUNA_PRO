...
- feat(multi-tenancy): Phase 2 second slice shipped -- `gdpr_requests` now real, row-level tenant-isolated. Closed the `?all=1` metadata residual Phase 1 had named but under-scoped: re-inspecting `GDPRHandler.download()` found it served the actual completed export FILE (full profile + event history + extension data, not just request metadata) to any `users.admin` caller by guessable sequential request id, with zero tenant check at all. New migration `202609111002_gdpr_requests_tenant_id.sql` adds `tenant_id`, backfilled via a real join to `local_users.tenant_id`. `gdpr.ListRequests` now takes and filters by `tenantID`; `GDPRHandler.download()` checks the row's own `tenant_id` against the caller's before the admin bypass, same 404-not-403 idiom. New adversarial tests (`TestGDPRHandler_DownloadCrossTenantExportReturns404`, `TestGDPRHandler_ListRequestsAllScopedToCallerTenant`) plus live verification against the real running binary: a real tenant-1 admin token correctly 404s downloading a tenant-2 export by id and never sees it in `?all=1`, while same-tenant list/download/export all continue to work. `go build`/`go vet`/`go test ./...` all clean. Also fixed in the same pass: `branding_settings`/`compliance_recordings` explicitly named as NOT Phase-2-shaped (per-instance singletons with a hardcoded `id=1` row shared by every tenant today -- a real Phase 3 per-tenant-config redesign, not an admin-bypass bug). Real, honest incident found and fully contained during this pass: an earlier throwaway live-verification boot (mail/SIP slice) isolated `SQLITE_PATH` but not `IDUNA_PRO_ROOT`, so two fake test-registration events leaked into the real production event log and were later replayed into production `local_users` by an unrelated restart -- caught immediately (both fake rows had `is_admin=0`, zero references elsewhere), removed directly from production (safe per `cmd/admin-grant`'s own "projector never rebuilds from scratch" property), confirmed clean via a full restart. Process fixed: any future throwaway boot fully isolates `IDUNA_PRO_ROOT` under `/tmp` (migrations symlinked read-only), not just the database path. See `docs/MULTI_TENANCY_NORTHSTAR.md`. (sess-20260905-0720-ec33e7c5)

- feat(multi-tenancy): Phase 2 first slice shipped -- `mail_account_credentials` + `sip_accounts` now real, row-level tenant-isolated. Found by direct code inspection right after Phase 1 landed: both tables already did real Go-side filtering by `owning_org_id`/`created_by` for a non-admin caller, but a `users.admin` caller bypassed it entirely -- `mail_accounts.go`'s `list()` had no `WHERE` clause at all, `revealPassword()` returned a live decrypted mailbox password to any admin regardless of tenant, and `sip_accounts.go`'s `upsert()`/`remove()` let an admin hijack or delete any tenant's phone extension outright. Same vulnerability class as the GDPR gap Phase 1 closed, one layer down. New migration `202609111001_mail_and_sip_tenant_id.sql` adds `tenant_id` to both tables, backfilled via a real join to `local_users.tenant_id` (not a bare constant). New `localUserTenantID` helper (`users.go`) for `sip_accounts.upsert`'s insert-or-update case, where no existing row's own `tenant_id` can be checked yet -- looks up the target's real tenant directly from `local_users`. Same 404-not-403 idiom throughout: a cross-tenant probe gets byte-for-byte the same response as a genuinely nonexistent uid. New adversarial tests (`TestMailAccountsHandler_AdminCannotRevealCrossTenantMailboxPassword`, `TestSipAccounts_AdminCannotSeeOrTouchCrossTenantAccount`) plus live verification against the real running binary: two real tenants, `GET /api/v1/sip-accounts` correctly scoped, cross-tenant `PUT`/`DELETE /api/v1/sip-accounts/{uid}` both 404 with the target row provably untouched (confirmed via direct SQL after), and a cross-tenant `GET /api/v1/mail-accounts/{uid}/reveal-password` returning 404 instead of leaking the real password. `go build`/`go vet`/`go test ./...` all clean. See `docs/MULTI_TENANCY_NORTHSTAR.md`. (sess-20260905-0720-ec33e7c5)

- feat(multi-tenancy): Phase 1 shipped -- real, row-level tenant isolation on `local_users`. New `tenants` table (seeded with tenant 1 = CarePyre) + `local_users.tenant_id` column (migrations `202609110001`/`202609110002`). `userlog.UserProjector`'s `GetByUID`/`GetByEmail`/`ListUsers`/`ScrubPII` all gained an explicit, compiler-enforced `tenantID int` parameter -- chosen over an implicit context-read specifically so a missing scope is a build error, not a silently-wrong default. New `tenant_id` JWT claim (`local_auth.go`/`register.go`) + `middleware.TenantIDFromContext` + `callerTenantID(r)` (defaults to tenant 1 when the claim is absent -- a real, temporary, explicitly-named fallback since agent/cookie tokens don't carry the claim yet, safe only because tenant 1 is the only real tenant that exists). Threaded through every real `local_users`-touching handler (`users.go`, `me.go`, `change_password.go`). Real, found-live security gap closed in the same pass, not deferred: `internal/gdpr`'s own Export/Delete pipeline took a bare `local_uid` with zero tenant check anywhere in the chain -- a tenant-A admin could otherwise export or permanently PII-scrub a tenant-B user. Fixed via a new `verifyTenantMembership` check + `gdpr.ErrNotFound` (mapped to a real 404, checked BEFORE any `gdpr_requests` row is even created, so a cross-tenant probe leaves no trace). New tests at every layer including a real trip-wire pinning the tenant-1 fallback. Live-verified end to end against the real running binary, not just `go test`: two real registered users, one moved into a hand-inserted second tenant, confirmed `GET /api/v1/users` correctly scoped, `GET /api/v1/users/{cross-tenant-uid}` and `POST /api/v1/gdpr/export {local_uid: cross-tenant}` both correctly 404. `go build`/`go vet`/`go test ./...` all clean. See `docs/MULTI_TENANCY_NORTHSTAR.md`. (sess-20260905-0720-ec33e7c5)

## 2026-09-11
- docs: real, spec-only Multi-Tenancy NORTHSTAR (`docs/MULTI_TENANCY_NORTHSTAR.md`). Founder real-time: "idunapro needs to become truely multi tenant currently its just a fork of iduna for carepyre." Checked directly, confirmed true: zero `tenant_id` anywhere in `internal/store`, ten `.go` files hardcode `carepyre`/`CarePyre` by name, one process/one SQLite file/one JWT key for everyone -- `organizations`/`org_id` is a real but WITHIN-tenant concept (CarePyre's own CP-HIPAA-3 agency-cluster hierarchy), not tenant isolation. Names why DB-per-install (the existing plan) doesn't scale to fast self-serve tenant creation, proposes single-process row-level isolation instead (a `tenants` table, `tenant_id` everywhere, a JWT claim, per-tenant config), 3 real enforcement-mechanism candidates named not chosen, a 5-phase plan starting with `local_users` alone and a real adversarial cross-tenant-read-refused test. No code. (sess-20260905-0720-ec33e7c5)
...
## 2026-09-07
- Provisioned and promoted the first real CarePyre admin account for garybifrost@gmail.com via the existing register + cmd/admin-grant flow. (sess-20260905-0720-ec33e7c5)
